...
int main(int argc, char** argv) {
    ...
    // A flat mirror is a perfect reflector (white albedo); its other material
    // fields are unused because it scatters rather than shades.
    material* mirrorMat = new material(vec3(1,1,1), 0, 0, 0, 0, vec3(1,1,1));
    ...
    while(read == "sphere" || read == "cube" || read == "mirror" || read == "plane") {
        if (read == "sphere") {
            ...
            objs.push_back(new box(vec3(x, y, z), s, matCopy(matName)));
        ...
        } else { // mirror: reflective plane (point + normal)
            float px, py, pz, nx, ny, nz;
            inFile >> px >> py >> pz >> nx >> ny >> nz;
            objs.push_back(new plane(vec3(px, py, pz), vec3(nx, ny, nz), mirrorMat));
        }
        inFile >> read;
    }
    inFile.close();
